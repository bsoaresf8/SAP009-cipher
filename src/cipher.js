const cipher = {
  encode: function (offset, string) {
    //Esse parametro está trazendo o valor do meu text area (string) e do offset (deslocamento)

    let TextoCriptografado = "";
    for (let i = 0; i < string.length; i++) {
      //O for está iterando(percorrendo) cada caractere da minha text area
      //console.log(string[i], "Esta na posição:", i); //Quando i for 0 ele vai verificar a letra que ta na posição 0, assim por diante
      const codigoAsciiLetra = string.charCodeAt(i);
      if (codigoAsciiLetra >= 65 && codigoAsciiLetra <= 90) {
        const CodigoDaLetraConvertida =
          ((codigoAsciiLetra - 65 + offset) % 26) + 65; //está guardando o código da letra convertida
  ...
  decode: function (offset, string) {
    ...
    for (let i = 0; i < string.length; i++) {
      ...
      if (codigoAsciiLetra >= 65 && codigoAsciiLetra <= 90) {
        const CodigoDaLetraConvertida =
          ((codigoAsciiLetra + 65 - offset) % 26) + 65;
        const LetraConvertida = String.fromCharCode(CodigoDaLetraConvertida);
